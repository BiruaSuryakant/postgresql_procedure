CREATE OR REPLACE PROCEDURE pinnacle_interim_database.sp_damagecodes(IN db_instance_id integer, IN remote_conn character varying, IN prm_date timestamp without time zone)
 LANGUAGE plpgsql
AS $procedure$  
...
DECLARE
	...
	table_name varchar(50) = 'damagecodes';
...
begin
	...
	delete from pinnacle_interim_database.damagecodes where instanceid = db_instance_id;
	...
	insert into pinnacle_interim_database.damagecodes(code,
	description, instanceid)
	select * from dblink(remote_conn,CONCAT('select code,
	description,', db_instance_id,' from proview.damagecodes'))
	AS P(code varchar(10), description varchar(50), instanceid int4);
	...
	select into sourcedbcount count(*) from dblink(remote_conn,'select code from proview.damagecodes')
	AS P(code varchar(10));

	--insert log table
	perform pinnacle_interim_database.add_log(db_instance_id,'damagecodes',sourcedbcount,
	...
	perform pinnacle_interim_database.add_log(db_instance_id,'sp_damagecodes',0,0,0,'Failure',0,concat(v_sqlstate,v_message,v_context));
